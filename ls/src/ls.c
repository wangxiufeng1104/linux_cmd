...
void mode2str(mode_t smode, char *mode, char *color)
{
    ...
    if (S_ISREG(smode))
        ...
        mode[0] = 'd', strcpy(color, "BLUE_HL");
    else if (S_ISBLK(smode))
        mode[0] = 'b';
    else if (S_ISCHR(smode))
        mode[0] = 'c';
#ifdef S_ISFIFO
    else if (S_ISFIFO(smode))
        mode[0] = 'p';
#endif
#ifdef S_ISLNK
    else if (S_ISLNK(smode))
        mode[0] = 'l', strcpy(color, "BLUE");
#endif
#ifdef S_ISSOCK
    else if (S_ISSOCK(smode))
        mode[0] = 's';
#endif
    else
        mode[i++] = '?';
    ...
    // 获取文件特殊权限
    if (smode & S_ISUID)
        mode[3] = (smode & S_IXUSR) ? 's' : 'S';
    if (smode & S_ISGID)
        mode[6] = (smode & S_IXGRP) ? 's' : 'S';
    if (smode & S_ISVTX)
        mode[9] = (smode & S_IXOTH) ? 't' : 'T';
    // + 配置颜色 [上面也有]
    if (mode[0] == '-')
    {
        if (strstr(mode, "x"))
            strcpy(color, "GREEN_HL");
        if (strstr(mode, "s") || strstr(mode, "S"))
            strcpy(color, "YELLOW_BG");
    }
    return;
}
// 获取友好的mtime
...
void myls(char *path, int a_flag)
{
    ...
    while (i < n - 1)
    {
        ...
        if (!lstat(ab_path, &st))
        {
            ...
            mode2str(st.st_mode, mode, color);
            // 2. 处理文件信息 [并设置颜色]
            mtim2str(&st.st_mtim, mtime, sizeof(mtime));
            // 3. 处理修改时间
            ...
            else if (!strcmp(color, "YELLOW_BG"))
